...
@phishing_detector_bp.route('/email/<int:email_id>/reanalyze', methods=['GET', 'POST'])
@login_required
def reanalyze_email(email_id):
    """Reanalyze an email that has already been classified"""
    try:
        email = Email.query.filter_by(id=email_id, user_id=current_user.id).first_or_404()
        
        # Check if email has been classified before
        classification = PhishingClassification.query.filter_by(email_id=email.id).first()
        
        # Keep track of the previous user feedback (if any)
        previous_feedback = None
        if classification:
            previous_feedback = classification.feedback
        ...
        new_classification = PhishingClassification(
            email_id=email.id,
            phishing_score=phishing_score,
            is_phishing=phishing_score > 0.5,
            features_json=json.dumps(features),
            classified_at=datetime.utcnow(),
            feedback=previous_feedback  # Preserve the user's previous feedback
        )
        db.session.add(new_classification)
        db.session.commit()
        
        flash(f"Email reanalyzed. New phishing score: {round(phishing_score * 100, 1)}%", "success")
        
        # Handle AJAX vs direct browser requests
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return jsonify({
                'success': True,
                'message': f'Email reanalyzed. New phishing score: {round(phishing_score * 100, 1)}%',
        ...
        logger.error(f"Error reanalyzing email: {str(e)}")
        db.session.rollback()
        
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return jsonify({'error': 'Could not reanalyze email'}), 500
        else:
            flash("Error reanalyzing email", "danger")
            return redirect(url_for('phishing_detector_bp.phishing_detection_dashboard'))